...
static double HDW_BME280_temperature;
static double HDW_BME280_pressure;
static double HDW_BME280_humidity;


static uint8_t HDW_BME280_poll_timer;
...
int BME280_Config (uint8_t osrs_t, uint8_t osrs_p, uint8_t osrs_h, uint8_t mode, uint8_t t_sb, uint8_t filter)
{
	...
	HDW_BME280_poll_timer = SYS_TIMER_create(TIMER_MODE_REPEATED);
	SYS_TIMER_start(&HDW_BME280_poll_timer, 2000);
...
void BME280_Measure (void)
{
	if (BMEReadRaw() == 0)
	{
		  if (tRaw == 0x800000) HDW_BME280_temperature = 0; // value in case temp measurement was disabled
		  else
		  {
			  HDW_BME280_temperature = (BME280_compensate_T_int32 (tRaw))/100.0;  // as per datasheet, the temp is x100
		  }

		  if (pRaw == 0x800000) HDW_BME280_pressure = 0; // value in case temp measurement was disabled
		  else
		  {
#if SUPPORT_64BIT
			  HDW_BME280_pressure = (BME280_compensate_P_int64 (pRaw))/256.0;  // as per datasheet, the pressure is x256

#elif SUPPORT_32BIT
			  HDW_BME280_pressure = (BME280_compensate_P_int32 (pRaw));  // as per datasheet, the pressure is Pa

#endif
		  }

		  if (hRaw == 0x8000) HDW_BME280_humidity = 0; // value in case temp measurement was disabled
		  else
		  {
			  HDW_BME280_humidity = (bme280_compensate_H_int32 (hRaw))/1024.0;  // as per datasheet, the temp is x1024
		  }
	}
	...
	{
		HDW_BME280_temperature = HDW_BME280_pressure = HDW_BME280_humidity = 0;
	}
}

void HDW_BME280_process(void)
{
	if (SYS_TIMER_expired(HDW_BME280_poll_timer))
	{
		if (HAL_I2C_IsDeviceReady(&hi2c1, BME280_ADDRESS, 3, 10) == HAL_OK)
		{
			BME280_Measure();
		}
	}
...
double HDW_BME280_get_temperature(void)
{
	return(HDW_BME280_temperature);
}
double HDW_BME280_get_humidity(void)
{
	return(HDW_BME280_humidity);
}
double HDW_BME280_get_pressure(void)
{
	return(HDW_BME280_pressure);
}
